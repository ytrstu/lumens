...
public class DatasourceParser implements ContentHandler
{
                                   ...
                                   org.xml.sax.Attributes attrs) throws org.xml.sax.SAXException
    {
        ...
        {
            handler.start_Datasources(attrs);
    ...
    private void dispatch(final boolean fireOnlyIfMixed) throws SAXException
    {
        ...
        if ("Name".equals(here))
        {
            ...
            handler.handle_Name(buffer.length() == 0 ? null : buffer.toString(),
                                attrs);
        } else if ("ID".equals(here))
        {
            if (fireOnlyIfMixed)
                throw new IllegalStateException("Unexpected characters() event! (Missing DTD?)");
            handler.handle_ID(buffer.length() == 0 ? null : buffer.toString(),
                              attrs);
        } else if ("ClassName".equals(here))
        {
            if (fireOnlyIfMixed)
                throw new IllegalStateException("Unexpected characters() event! (Missing DTD?)");
            handler.handle_ClassName(buffer.length() == 0 ? null : buffer.toString(),
                                     attrs);
        } else if ("Icon".equals(here))
        {
            if (fireOnlyIfMixed)
                throw new IllegalStateException("Unexpected characters() event! (Missing DTD?)");
            handler.handle_Icon(buffer.length() == 0 ? null : buffer.toString(),
                                attrs);
        } else if ("ComponentIcon".equals(here))
        {
            if (fireOnlyIfMixed)
                throw new IllegalStateException("Unexpected characters() event! (Missing DTD?)");
            handler.handle_ComponentIcon(buffer.length() == 0 ? null : buffer.toString(),
                                         attrs);
        } else
        {
            //do not care
